...
public class Chap6Ex6 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if((userId != null && id.equals(userId)) && (userPw != null && pw.equals(userPw))) {
			...
			// session.setAttribute(name, value) -> 세션에 값을 저장할 수 있음
		...
		// 세션에 isLogin 키가 없을 경우 nullPointerExeption 이 발생할 수 있음. 
		
		// getAttribute(name) -> 세션에 저장된 값을 불러올 수 있음. 
		...
		if(isLogin) {
			// 로그아웃 버튼을 출력
			out.print("<a href=\"/Market/chap/ex7\">로그아웃</a>");
		} else {
			// "로그인을 하세요." 메세지 출력
