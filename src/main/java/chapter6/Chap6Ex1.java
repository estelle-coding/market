...
public class Chap6Ex1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// setPath 메서드를 사용
		Cookie c1 = new Cookie("id", "quest"); // id = quest
		c1.setPath("/");
		// name=value;path=/
		...
		// setDomain 메서드를 사용하면 쿠키를 전달한 서버를 지정
		//
		// 예를 들어 www.domain.com을 인자로 넣으면 이 쿠키는 www.domain.com 서버로
		// 요청했을 때만 전달됨
		...
		c2.setDomain("192.168.20.115");
		response.addCookie(c2);
		
		// name=value;domain=192.168.20.115
		
		// 일반적으로 아래와 같은 형태로 쿠키를 만들고 유지시간만 설정
